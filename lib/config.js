...
function WombatRC(argv, cfg)
{
	argv = argv || { registry: 'default' };
	if (!(this instanceof WombatRC)) return new WombatRC(argv, cfg);

	this.config = cfg || rc('wombat', cloneDeep(DEFAULTS), null, toml.parse);  // injection for testing
