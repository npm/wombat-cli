...
var Registry = module.exports = function Registry(argv)
{
	...
	this.config = config;
	this.config.load(this.registry);
...
Registry.prototype.authed = function authed(opts, callback)
{
	...
	if (!token)
		return callback(new Error('you are not logged into the registry you are trying to use'));
	var hostname = opts.legacy ? this.registry : config.get('api');
